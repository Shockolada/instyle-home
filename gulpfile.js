...
gulp.task('server', function () {
  ...
  gulp.watch('source/img/icons/**/*.png', gulp.series('pngsprite', 'imagesmin')).on('change', server.reload);
  gulp.watch('source/fonts/**/*.*', gulp.series('copy')).on('change', server.reload);
});

/* PNG SPRITES */
gulp.task('pngsprite', function () {
  const spriteData = gulp.src('source/img/icons/**/*.png')                  // Путь к png иконкам
  .pipe(spriteSmith({
    ...
    retinaSrcFilter: 'source/img/icons/**/*/*@2x.png',                      // Где брать иконки Retina
    retinaImgPath: "../img/sprite@2x.png",                                  // Какой путь указывать для Retina спрайта
